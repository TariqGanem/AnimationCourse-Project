...
Eigen::MatrixXd igl::opengl::ViewerData::getV() {
	return V;
}

// Helper function to get the top the bounding box of a the current data
Eigen::Vector3f igl::opengl::ViewerData::getBoundings(int key /*0 for TOP, 1 for BOTTOM, 2 FOR CENTER*/) {
	Eigen::Vector3d m = V.colwise().minCoeff();
	Eigen::Vector3d M = V.colwise().maxCoeff();
	auto mid = key == 0 ? M(1) : key == 1 ? m(1) : (M(1) + m(1)) / 2;
	Eigen::RowVector3d getTop = Eigen::RowVector3d(((M(0) + m(0))) / 2, mid, (((m(2) + M(2))) / 2));
	Eigen::Vector4f to_ret = MakeTrans() * Eigen::Vector4f(getTop.x(), getTop.y(), getTop.z(), 1);
	return Eigen::Vector3f(to_ret[0], to_ret[1], to_ret[2]);
}
